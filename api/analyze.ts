...
- STRONG spoofing indicator: authentication results explicitly showing "dmarc=fail" (the overall DMARC verdict itself failing, not just a component), or "compauth=fail" / "compauth=softpass" with other red flags present.
- STRONG: Return-Path domain being unrelated to the From domain — e.g. From: vanta.com but Return-Path at a domain with no visible connection to vanta.com. A Return-Path on an ESP bounce subdomain of the sender's own domain (e.g. mail-bounces.vanta.com for a vanta.com sender) is normal practice for transactional email and is NOT a mismatch.
